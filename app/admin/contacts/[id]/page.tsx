...
export default function ContactDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    params.then(({ id }) => {
      fetch(`/api/admin/contacts/${id}`).then((r) => r.json()).then((data: ContactSubmission) => {
        setContact(data);
    ...
    <div className="max-w-2xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          <Link href="/admin/contacts" className="text-stone-500 hover:text-stone-900 transition-colors"><ArrowLeft className="w-5 h-5" /></Link>
          <h2 className="text-2xl font-bold text-stone-900">Message Details</h2>
        </div>
      ...
      <div className="bg-white rounded-2xl border border-stone-200 p-8 space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h3 className="text-xl font-bold text-stone-900">{contact.name}</h3>
            <p className="text-stone-500 text-sm">{new Date(contact.created_at).toLocaleString()}</p>
          </div>
        ...
        <div>
          ...
          <p className="text-stone-700 text-sm leading-relaxed bg-stone-50 rounded-xl p-4">{contact.message}</p>
        </div>
        ...
        <div className="flex items-center gap-4">
          ...
          <a href={`mailto:${contact.email}?subject=Re: ${contact.subject}`} className="px-6 py-2.5 border border-stone-300 text-stone-700 rounded-xl text-sm font-medium hover:bg-stone-50 transition-colors">
            Reply via Email
          </a>
        </div>
      </div>

      <ConfirmDialog isOpen={showDelete} title="Delete Message" message={`Delete message from "${contact.name}"? This cannot be undone.`}
